<?php
    ...
    if ($_SESSION['csrf'] === $_POST['csrf']) {
        ...
        if (isset($_POST['add_size'])) {
            $new_size_name = cleanInput($_POST['new_size_name']);
            $new_size_val = (int)$new_size_name;
            $stmt = $db->prepare('INSERT INTO Size_(SizeVal) VALUES (?)');
            $stmt->execute([$new_size_val]);
            header('Location: ../pages/admin_page.php');
            exit();
        }

        if (isset($_POST['delete_size'])) {
            $size_id_to_delete = cleanInput($_POST['size_id_to_delete']);
